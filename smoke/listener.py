...
async def main() -> None:
    ...
    channels = _list_env("TELEGRAM_LISTENER_MONITOR_CHANNELS")
    if not channels:
        raise RuntimeError(
            f"TELEGRAM_LISTENER_MONITOR_CHANNELS is required in {SMOKE_DIR / '.env'}"
        )
    ...
    try:
        ...
        listener = TelegramListener(
            client=client,
            channels=channels,
            image_channels=_list_env("TELEGRAM_LISTENER_IMAGE_CHANNELS"),
            translation_channels=_list_env("TELEGRAM_LISTENER_TRANSLATION_CHANNELS"),
            translation_target_language=os.getenv(
                "TELEGRAM_LISTENER_TRANSLATION_TARGET_LANGUAGE", "en"
            ),
            translation_timeout=float(
                os.getenv("TELEGRAM_LISTENER_TRANSLATION_TIMEOUT", "3")
            ),
            translation_max_concurrency=int(
                os.getenv("TELEGRAM_LISTENER_TRANSLATION_MAX_CONCURRENCY", "2")
            ),
            queue_maxsize=int(os.getenv("TELEGRAM_LISTENER_QUEUE_MAXSIZE", "1000")),
        )
